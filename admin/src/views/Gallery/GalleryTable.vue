    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div
            ...
            <div class="flex items-center gap-4">
                <span class="text-sm text-slate-500">Found <span class="font-semibold text-slate-900">{{ galleries.data
                    ? galleries.data.length : 0
                        }}</span> images</span>
            </div>

            <div class="relative">
                <input v-model="search" @input="filterGalleries"
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50 text-slate-600 uppercase text-xs font-semibold">
                    <tr>
                        ...
                        <TableHeadingCell class="px-6 py-4 border-b border-slate-200" field="alternative">
                            Alternative Text
                        </TableHeadingCell>
                ...
                <tbody v-else class="divide-y divide-slate-200">
                    <tr v-for="(gallery, index) in filteredGalleries" :key="gallery.id"
                        class="hover:bg-slate-50 transition-colors">
                        <td class="px-6 py-4 text-sm text-slate-600 font-medium">#{{ gallery.id }}</td>
                        ...
                        <td class="px-6 py-4">
                            <img class="w-16 h-16 object-cover rounded-lg" :src="gallery.image"
                                :alt="gallery.alternative" />
                        </td>
                                ...
                                <transition enter-active-class="transition duration-100 ease-out"
                                    ...
                                    <MenuItems
                                        ...
                                        <div class="px-1 py-1">
                                            ...
                                            <RouterLink :to="{ name: 'app.galleries.edit', params: { id: gallery.id } }"
                                                :class="[
...
const galleries = computed(() => store.state.galleries);
const galleryLoading = ref(true);

const filteredGalleries = computed(() => {
    if (!galleries.value.data) return []
    if (!search.value) return galleries.value.data

    return galleries.value.data.filter(gallery =>
        gallery.alternative.toLowerCase().includes(search.value.toLowerCase())
    )
})
...
function getGalleries(url = null) {
    store.dispatch("getGalleries").then(() => {
...
function deleteGallery(gallery) {
    ...
    store
        ...
        .then(() => {
            store.commit("showToast", "Image deleted successfully");
