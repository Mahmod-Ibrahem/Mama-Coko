        ...
        <div class="max-w-7xl mx-auto mb-6">
            <div class="flex items-center gap-3 mb-2">
                ...
                <h1 v-if="!loading" class="text-2xl md:text-3xl font-bold text-slate-800">
                    {{ gallery.id ? `Update Gallery Image` : 'Add New Gallery Image' }}
                </h1>
            </div>
            <p v-if="gallery.id && !loading" class="text-slate-500 ml-13">
                Editing: <span class="font-medium text-indigo-600">{{ gallery.alternative }}</span>
            </p>
        </div>
        ...
        <div class="max-w-7xl mx-auto">
            ...
            <form v-else @submit.prevent="onSubmit">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">
                                Alternative Text <span class="text-red-500">*</span>
                            </label>
                            <CustomInput v-model="gallery.alternative" :errors="errors.alternative"
                                placeholder="Enter image description for accessibility" />
                        </div>

                        <!-- Gallery Image -->
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">Gallery Image</label>
                            <div class="relative w-1/4">
                                <input type="file" id="gallery-image" class="hidden" accept="image/*"
                                    @change="e => handleFileChange(e.target.files[0])" />
                                <label for="gallery-image" class="flex flex-col items-center justify-center w-full h-80 
                                    ...
                                    <img v-if="imagePreview" :src="imagePreview" alt="Gallery Preview"
                                    ...
                                    <div v-else class="text-center p-6">
                                        ...
                                        <p class="mt-2 text-sm text-slate-500">Click to upload gallery image</p>
                                    </div>
                                </label>
                            </div>
                ...
                flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3 mt-6">
                    <RouterLink :to="{ name: 'app.galleries' }" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 
...
function onSubmit($event, close = false) {
    loading.value = true;
    errors.value = {};
    const action = gallery.value.id ? "updateGallery" : "createGallery";
    const successStatus = action === "updateGallery" ? 200 : 201;
    const successMessage =
        action === "updateGallery"
            ? "Gallery image has been successfully updated"
            : "Gallery image has been successfully created";

    store
        .dispatch(action, gallery.value)
        .then((response) => {
            loading.value = false;
            if (response.status === successStatus) {
                ...
                if (close) {
                    router.push({ name: "app.galleries" });
...
onMounted(() => {
    if (route.params.id) {
        loading.value = true;
        store
            .dispatch("getGallery", route.params.id)
            .then((response) => {
                loading.value = false;
                gallery.value = response.data;
